...
connection.query(makeUserTable, function (err, result) {
    ...
    let makeNoteTable = "CREATE TABLE IF NOT EXISTS `denote-schema`.`note` (`noteid` INT NOT NULL AUTO_INCREMENT,`reminder` TEXT NULL,`user_id` INT NULL,PRIMARY KEY (`noteid`),  `created_at` DATETIME NULL,`updated_at` DATETIME NULL,INDEX `fk_note_user_idx` (`user_id` ASC),CONSTRAINT `fk_note_user`FOREIGN KEY (`user_id`)REFERENCES `denote-schema`.`user` (`id`)ON DELETE NO ACTION ON UPDATE NO ACTION);"
